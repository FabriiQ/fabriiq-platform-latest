...
import { AttendanceCacheService } from './attendance-cache.service';
...
export class AttendanceService extends ServiceBase {
  ...
  async createAttendance(data: z.infer<typeof createAttendanceSchema>) {
    try {
      ...
      if (existingAttendance) {
        ...
        // Invalidate caches for this class and student
        AttendanceCacheService.invalidateClassCaches(data.classId);
        AttendanceCacheService.invalidateStudentCaches(data.studentId);
  ...
  async updateAttendance(data: z.infer<typeof updateAttendanceSchema>) {
    try {
      ...
      // Invalidate caches for this class and student
      AttendanceCacheService.invalidateClassCaches(existingAttendance.classId);
      AttendanceCacheService.invalidateStudentCaches(existingAttendance.studentId);
  ...
  async getAttendanceByQuery(query: z.infer<typeof attendanceQuerySchema>) {
    return AttendanceCacheService.getCachedRecords(
      query.classId,
      () => this._computeAttendanceByQuery(query),
      query.studentId,
      query.startDate,
      query.endDate,
  ...
  /**
   * Internal method to compute attendance records by query (without caching)
   * @param query Attendance query
   * @returns Attendance records
   */
  private async _computeAttendanceByQuery(query: z.infer<typeof attendanceQuerySchema>) {
    try {
  ...
  async getClassAttendanceStats(classId: string, startDate?: Date, endDate?: Date) {
    return AttendanceCacheService.getCachedClassStats(
      classId,
      () => this._computeClassAttendanceStats(classId, startDate, endDate),
      startDate,
      endDate,
  ...
  /**
   * Internal method to compute class attendance statistics (without caching)
   * @param classId Class ID
   * @param startDate Start date
   * @param endDate End date
   * @returns Attendance statistics
   */
  private async _computeClassAttendanceStats(classId: string, startDate?: Date, endDate?: Date) {
    try {
      const whereClause: any = {
        classId,
      };
      ...
      const [attendanceRecords, enrollments, classEntity] = await Promise.all([
      ...
      // Use a more efficient approach for calculating unique days
      const uniqueDates = new Set<string>();
      const statusCounts: Record<AttendanceStatusType, number> = {} as Record<AttendanceStatusType, number>;
      const studentRecordsMap = new Map<string, any[]>();

      // Single pass through attendance records for better performance
      for (const record of attendanceRecords) {
        ...
        if (!studentRecordsMap.has(record.studentId)) {
          studentRecordsMap.set(record.studentId, []);
        }
        studentRecordsMap.get(record.studentId)!.push(record);
      }

      const totalDays = uniqueDates.size;

      // Calculate per-student statistics more efficiently
      const studentStats = enrollments.map(enrollment => {
        const studentRecords = studentRecordsMap.get(enrollment.studentId) || [];

        const studentStatusCounts: Record<AttendanceStatusType, number> = {} as Record<AttendanceStatusType, number>;
        let presentCount = 0;

        for (const record of studentRecords) {
          studentStatusCounts[record.status] = (studentStatusCounts[record.status] || 0) + 1;
          if (record.status === AttendanceStatusType.PRESENT) {
            presentCount++;
          }
        }
  ...
  async markAttendance(data: {
    ...
    // Invalidate caches for this class and all affected students
    AttendanceCacheService.invalidateClassCaches(classId);
    records.forEach(record => {
      AttendanceCacheService.invalidateStudentCaches(record.studentId);
    });
