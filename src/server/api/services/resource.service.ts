...
export class ResourceService extends ServiceBase {
  ...
  /**
   * Gets resources available to a student
   * @param params Student resource query parameters
   * @returns Student accessible resources
   */
  async getStudentResources(params: {
    ...
    try {
      const { studentId, courseId, searchTerm, type, skip = 0, take = 50 } = params;
      ...
      // Build the where clause for resources
      const whereClause: any = {
        status: SystemStatus.ACTIVE,
        OR: [
          // Resources shared with the student's classes
          {
            settings: {
              path: ['courseId'],
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to get student resources",
        cause: error,
      });
    }
  }
