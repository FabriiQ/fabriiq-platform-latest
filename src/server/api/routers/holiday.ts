...
export const holidayRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Authorization check
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.CREATE_HOLIDAY)) {
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Authorization check
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.UPDATE_HOLIDAY)) {
        throw new TRPCError({ code: "UNAUTHORIZED" });
      }
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Authorization check
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.DELETE_HOLIDAY)) {
        throw new TRPCError({ code: "UNAUTHORIZED" });
      }
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      // Authorization check
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.VIEW_HOLIDAYS)) {
        throw new TRPCError({ code: "UNAUTHORIZED" });
      }
