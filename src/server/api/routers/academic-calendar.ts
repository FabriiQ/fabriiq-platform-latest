...
const createEventSchema = z.object({
  ...
  academicCycleId: z.string().min(1, "Academic cycle is required"),
...
export const academicCalendarRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.CREATE_ACADEMIC_EVENT)) {
        throw new TRPCError({ 
          code: "FORBIDDEN",
      ...
      return service.createAcademicEvent({
        ...input,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.UPDATE_ACADEMIC_EVENT)) {
        throw new TRPCError({ 
          code: "FORBIDDEN",
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.DELETE_ACADEMIC_EVENT)) {
        throw new TRPCError({ 
          code: "FORBIDDEN",
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      if (!hasCalendarPermission(ctx.session.user.userType as UserType, CalendarAction.VIEW_ACADEMIC_EVENTS)) {
        throw new TRPCError({ 
          code: "FORBIDDEN",
