...
export function ModeSelector({ onModeChange, className }: ModeSelectorProps) {
  ...
  const handleModeSelect = (mode: TeacherMode) => {
    const newMode = selectedMode?.id === mode.id ? null : mode;
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={cn('p-2 rounded-lg', selectedMode.bgColor)}>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-sm">{selectedMode.label}</h3>
                    <Badge variant="secondary" className="text-xs">Active</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    All responses will be generated in this mode
                  </p>
                </div>
              </div>
              <Button
              ...
              {modes.map((mode) => (
                ...
                >
                  <div className="flex items-center gap-2 w-full">
                    <div className={selectedMode?.id === mode.id ? "text-primary-foreground" : mode.color}>
                      {mode.icon}
                    </div>
