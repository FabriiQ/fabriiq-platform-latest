...
export default function CreateAcademicCyclePage() {
  ...
  // For system admin, get the first available institution
  const { data: institution, isLoading: isLoadingInstitution } = api.institution.getCurrent.useQuery(
    undefined,
    { enabled: !!user && user.userType === 'SYSTEM_ADMIN' }
  );
  ...
  // Update form values when user and institution are loaded
  React.useEffect(() => {
  ...
  const onSubmit = async (data: AcademicCycleFormValues) => {
    if (!user?.id || !institution?.id) {
      toast({
        title: 'Error',
        description: 'User or institution information is missing. Please try again.',
        variant: 'error',
      });
      return;
    }
    ...
    return (
      ...
      >
        <Card>
          ...
          <CardContent>
            <p>You need to be logged in with an institution to create an academic cycle.</p>
          </CardContent>
        </Card>
      </PageLayout>
    );
