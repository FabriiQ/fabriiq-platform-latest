...
export default function EditAcademicCyclePage() {
  ...
  // Early return if params is null or id is missing
  if (!params?.id) {
    return <div>Invalid academic cycle ID</div>;
  }

  const academicCycleId = params.id as string;
  ...
  return (
    <PageLayout
      ...
      breadcrumbs={[
        ...
        { label: academicCycle?.name || 'Edit', href: `/admin/system/academic-cycles/${academicCycleId}` },
        { label: 'Edit', href: '#' },
      ]}
      actions={
        <Button
